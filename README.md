...
def calculate_wrapped_phase(I1, I2, I3):
    ...
    """
    phase_wrapped = np.arctan2(np.sqrt(3) * (I1 - I3), 2 * I2 - I1 - I3)
    return np.mod(phase_wrapped, 2 * np.pi)
...
def process_phase_images(image_paths, Scalelength=0.062, roi=None):
    """
    ...
    # Phase processing
    phase_wrapped = calculate_wrapped_phase(I1_roi, I2_roi, I3_roi)
    phase_unwrapped = unwrap_phase(phase_wrapped)

    # Convert to depth
    Z = compute_depth(phase_unwrapped, Scalelength)
...
def plot_results(results):
    ...
    phase_wrapped = results['phase_wrapped']
    phase_unwrapped = results['phase_unwrapped']
    Z = results['Z']
    plane = results['plane']
    ...
    for i in range(8):
        ...
        cbar.set_label(colorbar_labels[i], rotation=270, labelpad=15)
    ...
    surf = ax.plot_surface(X[::skip, ::skip], Y[::skip, ::skip], Z_flattened[::skip, ::skip],
    ...
    fig.colorbar(surf, ax=ax, shrink=0.6, pad=0.1, label='Height (µm)')
    plt.tight_layout()
    plt.show()
...
if __name__ == "__main__":
    ...
    """
        'C:\\Users\\Jiang\\Desktop\\sample\\1.tiff', 
        'C:\\Users\\Jiang\\Desktop\\sample\\2.tiff',
        'C:\\Users\\Jiang\\Desktop\\sample\\3.tiff',
    """
