...
```bash
def unwrap_LS_FD_FFT(phase_wrap):
    M, N = phase_wrap.shape

    # Finite differences (wrapped)
    edx = np.diff(phase_wrap, axis=1)
    edx = wrap_to_pi(edx)
    edx = np.hstack([edx, edx[:, -1:]])

    edy = np.diff(phase_wrap, axis=0)
    edy = wrap_to_pi(edy)
    edy = np.vstack([edy, edy[-1:, :]])

    # Compute Laplacian (rho)
    rho = np.zeros_like(phase_wrap)
    rho[:-1, :-1] = (
        edx[:-1, :-1] - edx[:-1, 1:] +
        edy[:-1, :-1] - edy[1:, :-1]
    )
    ...
    # FFT frequencies
    fx = np.fft.fftfreq(N).reshape(1, N)
    fy = np.fft.fftfreq(M).reshape(M, 1)
    fx = 2j * np.pi * fx
    fy = 2j * np.pi * fy
    f = fx**2 + fy**2
    ...
    phi_fft = rho_fft / f
    phi_fft[0, 0] = 0  # zero DC
    phi = np.real(np.fft.ifft2(phi_fft))

    # Correct DC offset to match first pixel
    phi += phase_wrap[0, 0] - phi[0, 0]
    return phi
